package org.flixel
{
	...
	public class FlxObject extends FlxBasic
	{
		...
		public var scrollOffset:FlxPoint;
		/**
		...
		public function FlxObject(X:Number=0,Y:Number=0,Width:Number=0,Height:Number=0)
		{
			...
			scrollOffset = new FlxPoint(0.0, 0.0);
			_flicker = false;
			_flickerTimer = 0;
		...
		public function flickerUpdate():void
		{
			if(_flickerTimer != 0)
			{
				if(_flickerTimer > 0)
		...
		override public function drawDebug(Camera:FlxCamera=null):void
		{
			...
			var boundingBoxX:Number = x - int((Camera.scroll.x*scrollFactor.x)+scrollOffset.x); //copied from getScreenXY()
			var boundingBoxY:Number = y - int((Camera.scroll.y*scrollFactor.y)+scrollOffset.y);
		...
		public function overlapsAt(X:Number,Y:Number,ObjectOrGroup:FlxBasic,InScreenSpace:Boolean=false,Camera:FlxCamera=null):Boolean
		{
			...
			if(Camera == null)
				Camera = FlxG.camera;
			var objectScreenPos:FlxPoint = object.getScreenXY(null,Camera);
			_point.x = X - int((Camera.scroll.x*scrollFactor.x) + scrollOffset.x); //copied from getScreenXY()
			_point.y = Y - int((Camera.scroll.y*scrollFactor.y) + scrollOffset.y);
		...
		public function getScreenXY(Point:FlxPoint=null,Camera:FlxCamera=null):FlxPoint
		{
			...
			if(Camera == null)
				Camera = FlxG.camera;
			Point.x = x - int((Camera.scroll.x*scrollFactor.x) + scrollOffset.x);
			Point.y = y - int((Camera.scroll.y*scrollFactor.y) + scrollOffset.y);
