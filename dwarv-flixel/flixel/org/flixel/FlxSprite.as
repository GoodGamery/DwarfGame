package org.flixel
{
	...
	public class FlxSprite extends FlxObject
	{
		...
		*/
		...
		public var _defaultColorTransform:ColorTransform;
		...
		public function boundGraphic(xoff:int, yoff:int, width:int,  height:int):void
		{
			if (_flipped)
			{
				width *= 2;
			...
			for (var x:int = 0; x < width; x++)
			{
				...
				{
					_pixels.fillRect(new Rectangle(offset.x + (x * xoff), offset.y + (y * yoff), this.width, this.height), 0xFFFFFFFF);
				}
			}
		...
		public function SetRigid(r:Boolean):void
		{
			var i:int = 0;
			if (r == false)
				i = 1;
		...
		public override function flickerUpdate():void
		{
			if(_flickerTimer != 0)
			{
				if(_flickerTimer > 0)
				{
					...
					if(_flickerTimer <= 0)
					{
						_colorTransform = _defaultColorTransform;
						calcFrame();
		...
		public var _bDangerFlicker:Boolean = false
		public var _cFlickerColor:uint = 0x00000000;
		public var _nFlickerRate:Number = 0.05;
		public var _nFlickerTick:Number = 0;
		public var _bPixelFix:Boolean = true;
		override public function draw():void
		{
			if(_flickerTimer != 0)
			{
				_nFlickerTick -= FlxG.elapsed;
				if (_nFlickerTick <= 0)
				{
					_flicker = !_flicker;
					_nFlickerTick += _nFlickerRate;
					
					if (_cFlickerColor != 0x00000000)
					{
						if (_flicker)
						{
							_colorTransform = new ColorTransform();
							_colorTransform.color = _cFlickerColor;
							calcFrame();
						}
						else
						{
				...
				if (_flicker && _cFlickerColor == 0x00000000)
				{
					return;
				}
			}
			else if (_bDangerFlicker)
			{
				_nFlickerTick -= FlxG.elapsed;
				...
				if (_nFlickerTick <= 0)
				{
					...
					if (_flicker)
					{
						_nFlickerTick += _nFlickerRate;
						_colorTransform = new ColorTransform();
						_colorTransform.color = 0xFFBB0000;
						calcFrame();
					}
					else
					{
						_nFlickerTick += _nFlickerRate * 5;
						_colorTransform = _defaultColorTransform;
						calcFrame();
					}
				}
			...
			while(i < l)
			{
				...
				if (_bPixelFix == false)
				{
					...
					//_point.x = int(_point.x);
					//_point.y = int(_point.y);
				}
				else
				{
					// try 1
					//_point.x = Math.round(x - camera.scroll.x * scrollFactor.x) - offset.x;
					//_point.y = Math.round(y - camera.scroll.y * scrollFactor.y) - offset.y;
					// try 2
					_point.x = (Math.floor(x - camera.scroll.x * scrollFactor.x) - offset.x) + 1;
					_point.y = (Math.floor(y - camera.scroll.y * scrollFactor.y) - offset.y) + 1;
