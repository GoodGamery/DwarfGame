package
{
	...
	public class Player extends FlxSprite
	{
		...
		public var bWasWindJump:Boolean = false;
		
		public var nWaterBoostLateral:Number = 1;
		public var nWaterBoostVertical:Number = 1;
		
		public var nWaterBoostLateralCap:Number = 2;
		public var nWaterBoostVerticalCap:Number = 2;
		
		public var nWaterBoostLateralSpurt:Number = 0.75;
		public var nWaterBoostVerticalSpurt:Number = 0.75;
		
		public var nWaterBoostLateralDecay:Number = (nWaterBoostLateralCap - 1) * (1 / (0.5)); // 3 seconds
		public var nWaterBoostVerticalDecay:Number = (nWaterBoostVerticalCap - 1) * (1 / (0.5)); // 3 seconds 
		...
		public var bSpurtRecharged:Boolean = true;
		public var bJumpCanBeCut:Boolean = false;
		public var nShootRecharged:Number = 1;
		public var nShootRechargeSpeed:Number = 0.025;//0.05;// 0.1;
		...
		public var bSwimSound:Boolean = false;
		override public function update():void
		{
			...
			if (iLastFluid == 0 && fluid == 1)
			{
				...
				if (facing == LEFT)
					(Util.GetInvisibleSpriteByName(parent.arrayParticles, "Splash") as Splash).Reuse(this.x - 8, (ycheck - 1) * 30);
				else
					(Util.GetInvisibleSpriteByName(parent.arrayParticles, "Splash") as Splash).Reuse(this.x - 4, (ycheck - 1) * 30);
			...
			else
			{
				...
				else if (fluid > 0 && bReallyInWater)
				{
					...
					if (bLeftButton && !bRightButton)
					{
						...
						if (bJumpButton && bSpurtRecharged) // Swim 1
						{
							if(fluid > 1)
								SwimSound(1);
					...
					else if (!bLeftButton && bRightButton)
					{
						...
						if (bJumpButton && bSpurtRecharged) // Swim 2
						{
							if(fluid > 1)
								SwimSound(2);
					...
					if (bDiveButton && !bUpButton)
					{
						...
						if (bJumpButton && bSpurtRecharged) // Swim 3
						{
							if(fluid > 1)
								SwimSound(3);
					...
					else if (fluid == 2 && !bDiveButton && bUpButton && bJumpButton && bSpurtRecharged) // Swim 4
					{
						if(fluid > 1)
							SwimSound(4);
					...
					else if (fluid == 1 && velocity.y <= 0 && int(this.y) % 30 <= 2 && bJumpButton != true)
					{
						nWaterBoostVertical = 1;
						velocity.y = 0;
				...
				else
				{
					if (!bShootButton && !bTossButton)
					{
						...
						//trace(nShootRecharged.toString());
					}
				}
